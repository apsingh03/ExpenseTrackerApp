...
const getUserLeaderBoard = async (req, res) => {
    ...
    const userTableJoiningExpenseTable = await Users.findAll({ 
        attributes : ['id' , 'fullname', [Sequelize.fn("sum" , Sequelize.col("expenses.money")) , "total_cost"]  ],
        include : [
            {
                model : Expenses,
                attributes : [],
    ...
    const expensesQuery = await Expenses.findAll({ 
        ...
        attributes : ['user_id' , [ Sequelize.fn("sum" , Sequelize.col("money")) , "total_cost"] ],
        include : [ 
            {
                model : Users , 
    ...
    res.status(200).send({msg : "its working" , usersQuery });
